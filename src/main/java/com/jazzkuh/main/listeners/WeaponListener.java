...
public class WeaponListener implements Listener {
    ...
    public void onEntityDamage(EntityDamageByEntityEvent event) {
        ...
        if (event.getDamager().getType() == EntityType.SNOWBALL) {
            damageEvent(event, player, "deserteagle");
            damageEvent(event, player, "magnum44");
            damageEvent(event, player, "waltherp99");
            damageEvent(event, player, "glock19");
            damageEvent(event, player, "m16a4");
        }
    }*/
   ...
   /* private void damageEvent(EntityDamageByEntityEvent event, Player player, String type) {
        if (event.getDamager().getName().contains(type)) {
            event.setCancelled(true);
